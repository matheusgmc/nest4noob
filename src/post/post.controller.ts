...
export class PostController {
  ...
  @Get("feed/")
  async getPublishedPosts(@Query("search") search: string): Promise<PostModel[]> {
  ...
  @Get(":id")
  async getPostById(@Param("id", new ParseIntPipe()) id: number): Promise<PostModel | null> {
  ...
  async createDraft(@Body() postData: CreatePostDTO): Promise<PostModel> {
    ...
    if (await this.userService.users({
      where: {
        email: authorEmail
      }
    })) {
